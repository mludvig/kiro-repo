...
class NotificationService:
    ...
    def send_success_notification(self, release: ReleaseInfo, message: str = None) -> None:
        ...
        try:
            self.sns_client.publish(
                TopicArn=self.success_topic_arn,
                Subject=subject,
                Message=message
            )
        except ClientError as e:
    ...
    def send_failure_notification(self, error: Exception, context: str = None) -> None:
        ...
        message = f"The Kiro Debian Repository Manager encountered an error:\n\n"
        ...
        try:
            self.sns_client.publish(
                TopicArn=self.failure_topic_arn,
                Subject=subject,
                Message=message
            )
        except ClientError as e:
    ...
    def send_no_update_notification(self, current_version: str) -> None:
        ...
        subject = f"Kiro Debian Repository - No Update Needed"
